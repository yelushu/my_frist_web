...
def edit():
    ...
    user_info = g.current_user#前面已经获取信息，不需要返回数据库再次更新
    user_info.nickname = nickname#哪里更新，就更改哪里
...
def resetPwd():
    ...
    user_info.login_pwd = UserService.genePwd( new_password,user_info.login_salt )#为了安全，验证该用户登陆密码是否正确

    db.session.add( user_info )
    db.session.commit()

    response = make_response(json.dumps( resp ))#防止更新完密码后就退出，要修改cook（因为系统实时核对cook）
    response.set_cookie(app.config['AUTH_COOKIE_NAME'], '%s#%s' % (
        UserService.geneAuthCode(user_info), user_info.uid), 60 * 60 * 24 * 120)  # 保存120天
    return response
