...
def index():
    resp_data={}
    req=request.values
    page=int(req['page']) if ('p'in req and req['p'])else 1

    query=User.query
    if 'mix_kw' in req:
        rule = or_(User.nickname.ilike("%{0}%".format(req['mix_kw'])), User.mobile.ilike("%{0}%".format(req['mix_kw'])))#查询规则
